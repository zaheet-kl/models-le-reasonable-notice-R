reasonable_notice_model <- readRDS(prophetPath('models', 'caEmploymentForesight', 'reasonableNotice', 'rn_best.rds'))
reasonable_notice_performance_model <- readRDS(prophetPath('models', 'caEmploymentForesight', 'reasonableNotice', 'rn_without_performance.rds'))

reasonable_notice_anchor_prediction <- function(pred_full, df){
...
reasonable_notice_transform_data <- function(df){
...
reasonable_notice_factors <- function(){
  labels <- reasonable_notice_model$var.names
  return(gsub('^X', '', labels))
}

reasonable_notice_performance_factors <- function(){
  labels <- reasonable_notice_performance_model$var.names
  return(gsub('^X', '', labels))
}


reasonable_notice_predict <- function(df) {

  df <- reasonable_notice_transform_data(df)

  pred_full <- predict(reasonable_notice_model, n.trees=1500, newdata=df)
  pred_anchor <- reasonable_notice_anchor_prediction(pred_full, df)$pred
...
reasonable_notice_performance_predict <- function(df) {
  
  df <- reasonable_notice_transform_data(df)
  
  pred_full <- predict(reasonable_notice_performance_model, n.trees=1500, newdata=df)
  pred_anchor <- reasonable_notice_anchor_prediction(pred_full, df)$pred
